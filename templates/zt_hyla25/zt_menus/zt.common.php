...
class MenuSystem
{
	...
	*/
	...
	function showMegaMenu($per, $pid, $level)
	{
		if(@$this->_nav[$pid])
		{
			...
			foreach($this->_nav[$pid] as $menu) 
			{
				...
				if(@$this->_nav[$menu->id]){$class = "item hasChild"; $id = "menu-".$menu->id;}
				else {$class = "item"; $id = "";}
	...
	function genMegaTypeNormal($row, $level = 0)
	{
		...
		//split menu-title
		if($group){
			$title1 = ($stitle !== false) ? JString::substr($row->title, 0, 1).'<span>'.JString::substr($row->title, 1).'</span>' : JString::substr($row->title, 1);
			$name			= '<span class="menu-title">'.$title1.'</span>';
		}else{
